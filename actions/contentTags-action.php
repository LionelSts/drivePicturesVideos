...
$filesData = [];
$page = $_POST['page'];
foreach($files as $file){                       // On stock toutes les informations convenablement en séparant le nom du fichier (qui est aussi son ID) de l'extension
    $filesData[] = explode(".", $file);
}
$link = mysqli_connect("127.0.0.1", "root", "", "drivelbr");    // connexion à la base de données
$link->query('SET NAMES utf8');
foreach ($filesData as $file){                                              // On supprime toutes les lignes tags existantes pour ces fichiers
...
$liste = "";
foreach ($filesData as $file){                                              //  On attribue les tags pour chaque fichier
